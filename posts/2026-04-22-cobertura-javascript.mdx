...
image: /covers/2026-04-22-cobertura-javascript.png
---
...
Se o teste chama `greet({ name: 'Ana' })`, tanto line coverage quanto statement coverage reportam 100% — todas as linhas e todos os statements executaram. Mas branch coverage reporta 50%. Existem dois caminhos no `if`, e o teste só cobriu um deles. O caminho "usuário indefinido" nunca foi exercitado.

Essa distinção importa porque, na prática, bugs vivem nos caminhos não tomados. Branch coverage captura isso; line coverage, não.

![Diagrama 1 — As quatro métricas em um snippet: a função `greet` com anotações sobre statements, branches, functions e lines; line coverage 100% pode coexistir com branch coverage 50%.](/images/2026-04-22-cobertura-javascript/Diagrama%201%20%C2%B7%20As%20quatro%20m%C3%A9tricas.png)
...
![Diagrama 2 — Os dois paradigmas lado a lado: instrumentação (A) versus observação do runtime com profiler (B); onde a informação nasce — no código em A, no motor em B.](/images/2026-04-22-cobertura-javascript/Diagrama%202%20%C2%B7%20Dois%20paradigmas.png)
...
```typescript
function greet(user) {
  cov[file].f[0]++; // função greet foi chamada
...
![Diagrama 3 — V8 coverage antes e depois do AST remapping: v8-to-istanbul (pré-3.2) versus ast-v8-to-istanbul com enumeração orientada ao AST (3.2+), inversão do sentido do raciocínio.](/images/2026-04-22-cobertura-javascript/Diagrama%203%20%C2%B7%20AST%20remapping.png)
